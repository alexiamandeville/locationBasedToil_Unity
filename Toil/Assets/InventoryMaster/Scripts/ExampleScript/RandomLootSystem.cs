...
public class RandomLootSystem : MonoBehaviour
{
    ...
    void Start()
    {
		...
		//GameObject randomLootItem = (GameObject)Instantiate(inventoryItemList.itemList[randomNumber].);
               
		//PickUpItem item = randomLootItem.AddComponent<PickUpItem>();
        //item.item = inventoryItemList.itemList[randomNumber];
		collectItemName.text = inventoryItemList.itemList[randomNumber].itemName;
